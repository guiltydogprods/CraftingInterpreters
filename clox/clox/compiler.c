...
static void method() {
	...
	if (parser.previous.length == 4 && memcmp((char*)parser.previous.start, "init", 4) == 0) {
...
static void string(bool canAssign) {
	...
	Value objVal = OBJ_VAL((Obj*)objString);
	emitConstant(objVal);
...
static void super_(bool canAssign) {
	...
	consume(TOKEN_IDENTIFIER, "Expect superclass method anem.");
