...
static void defineNative(const char* name, NativeFn function) {
	push(OBJ_VAL((Obj*)copyString(name, (int32_t)strlen(name))));
...
static bool call(ObjClosure* closure, int32_t argCount) {
	if (argCount != closure->function->arity) {
		runtimeError("Expected %d arguments but got %d.", closure->
...
static bool invokeFromClass(ObjClass* klass, ObjString* name, int32_t argCount) {
	Value method;
...
static bool invoke(ObjString* name, int32_t argCount) {
